...
extern "C" namespace meevax::io
{
  ...
  {
    return kernel::car(args).is<kernel::input_file>() ? kernel::true_object : kernel::false_object;
  }

  NATIVE(is_output_file)
  {
    return kernel::car(args).is<kernel::output_file>() ? kernel::true_object : kernel::false_object;
  }

  NATIVE(open_input_file)
  {
    return kernel::make<kernel::input_file>(
             kernel::car(args).as<kernel::string>()
           );
  }
  ...
  {
    kernel::car(args).as<kernel::input_file>().close();
    return kernel::unspecified;
  }

  NATIVE(close_output_file)
  {
    kernel::car(args).as<kernel::output_file>().close();
    return kernel::unspecified;
  }
} // extern "C"
