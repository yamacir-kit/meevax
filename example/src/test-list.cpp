...
BOOST_FIXTURE_TEST_SUITE(constructors, fixture); namespace
{
  BOOST_AUTO_TEST_CASE(cons_)
  {
    // (cons 'a '()) => (a)
    let x1 = cons(a, unit);
    BOOST_CHECK(lexical_cast<std::string>(x1) == "(a)");

    // (cons '(a) '(b c d)) => ((a) b c d)
    let x2 = cons(list(a), list(b, c, d));
    PRINT(lexical_cast<std::string>(x2));
    BOOST_CHECK(lexical_cast<std::string>(x2) == "((a) b c d)");

    // (cons "a" '(b c)) => ("a" b c)
    let x3 = cons(make<string>("a"), list(b, c));
    PRINT(lexical_cast<std::string>(x3));
    BOOST_CHECK(lexical_cast<std::string>(x3) == "(\"a\" b c)");

    // (cons 'a 3) => (a . 3)
    let x4 = cons(a, make<exact_integer>(3));
    PRINT(lexical_cast<std::string>(x4));
    BOOST_CHECK(lexical_cast<std::string>(x4) == "(a . 3)");

    // (cons '(a b) 'c) => ((a b) . c)
    let x5 = cons(list(a, b), c);
    PRINT(lexical_cast<std::string>(x5));
    BOOST_CHECK(lexical_cast<std::string>(x5) == "((a b) . c)");
  }
  ...
  BOOST_AUTO_TEST_CASE(list_)
  {
    ...
    BOOST_CHECK(lexical_cast<std::string>(x1) == "(a 7 c)");

    // (list) => ()
    let x2 = list();
    BOOST_CHECK(lexical_cast<std::string>(x2) == "()");
  }
  ...
  BOOST_AUTO_TEST_CASE(xcons_)
  {
    ...
    BOOST_CHECK(lexical_cast<std::string>(x1) == "(a b c)");
  }
  ...
  BOOST_AUTO_TEST_CASE(make_list_)
  {
    ...
    BOOST_CHECK(lexical_cast<std::string>(x1) == "(c c c c)");
  }
  ...
  BOOST_AUTO_TEST_CASE(list_tabulate_)
  {
    ...
    BOOST_CHECK(lexical_cast<std::string>(x1) == "(0 1 2 3)");
  }
  ...
  BOOST_AUTO_TEST_CASE(list_copy_)
  {
    let x1 = list(a, b, c);
    let x2 = list_copy(x1);
    BOOST_CHECK(lexical_cast<std::string>(x2) == "(a b c)");
