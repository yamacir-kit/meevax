...
(define eval
  (lambda (e a)
    ...
    (define zip
      (lambda (x y)
        (if (and (null? x) (null? y)) '()
            (if (and (pair? x) (pair? y)) (cons (list (car x) (car y)) (zip  (cdr x) (cdr y))) '()))))
    (define assoc
      (lambda (x y)
        (if (null? x) '()
            (if (null? y) x
    ...
    (define evcon
      (lambda (c a)
        (if (eval (caar c) a) (eval (cadar c) a) (evcon  (cdr c) a))))
    (define evlis
      (lambda (m a)
        (if (null? m) '() (cons (eval (car m) a) (evlis (cdr m) a)))))
    (if (not (pair? e)) (assoc e a)
        (if (not (pair? (car e)))
            (if (eq? (car e) 'quote) (cadr e)
                (if (eq? (car e) 'atom) (not (pair? (eval (cadr e) a)))
                    (if (eq? (car e) 'eq) (eq? (eval (cadr  e) a) (eval (caddr e) a))
                        (if (eq? (car e) 'car) (car (eval (cadr e) a))
                            (if (eq? (car e) 'cdr) (cdr (eval (cadr e) a))
                                (if (eq? (car e) 'cons) (cons (eval (cadr e) a) (eval (caddr e) a))
                                    (if (eq? (car e) 'cond)
                                        (evcon (cdr e) a)
