...
namespace meevax::inline kernel
{
  auto resolve(char const* name) -> void *
  {
    ...
    auto static const registry = std::unordered_map<std::string, procedure::signature>
    {
      ...
      { "list?",                         [](let const& xs) { return make<bool>(is_list(car(xs))); } },
      { "list",                          [](let const& xs) { return xs; } },
      { "make-list",                     [](let const& xs) { switch (length(xs | as_proper_list)) { case 1: return make_list(exact_integer_cast<std::size_t>(car(xs))); case 2: return make_list(exact_integer_cast<std::size_t>(car(xs)), cadr(xs)); default: throw error(make<string>("procedure make-list takes one or two arugments, but got"), xs); } } },
      { "iota",                          [](let const& xs) { switch (length(xs | as_proper_list)) { case 1: return iota(exact_integer_cast<std::size_t>(car(xs))); case 2: return iota(exact_integer_cast<std::size_t>(car(xs)), cadr(xs)); case 3: return iota(exact_integer_cast<std::size_t>(car(xs)), cadr(xs), caddr(xs)); default: throw error(make<string>("procedure iota takes one to three arugments, but got"), xs); } } },
      ...
      { "null-list?",                    [](let const& xs) { if (is_list(car(xs)) or is_circular_list(car(xs))) { return make<bool>(car(xs).is<null>()); } else { throw error(make<string>("procedure null-list? takes a proper-list or a circular-list, but got"), xs); } } },
      ...
      { "append",                        [](let const& xs) { return std::accumulate(std::ranges::begin(xs | as_proper_list), std::ranges::end(xs | as_proper_list), unit, [](let const& x, let const& y) { return append(x, y); }); } },
      ...
      { "concatenate",                   [](let const& xs) { return std::accumulate(std::ranges::begin(car(xs) | as_proper_list), std::ranges::end(car(xs) | as_proper_list), unit, [](let const& x, let const& y) { return append(x, y); }); } },
      ...
      { "+",                             [](let const& xs) { return std::accumulate(std::ranges::begin(xs | as_proper_list), std::ranges::end(xs | as_proper_list), e0, std::plus()); } },
      { "*",                             [](let const& xs) { return std::accumulate(std::ranges::begin(xs | as_proper_list), std::ranges::end(xs | as_proper_list), e1, std::multiplies()); } },
      { "-",                             [](let const& xs) { if (cdr(xs).is<pair>()) { return std::accumulate(std::next(std::ranges::begin(xs | as_proper_list)), std::ranges::end(xs | as_proper_list), car(xs), std::minus()); } else { return e0 - car(xs); } } },
      { "/",                             [](let const& xs) { if (cdr(xs).is<pair>()) { return std::accumulate(std::next(std::ranges::begin(xs | as_proper_list)), std::ranges::end(xs | as_proper_list), car(xs), std::divides()); } else { return e1 / car(xs); } } },
      ...
      { "gcd",                           [](let const& xs) { switch (length(xs | as_proper_list)) { case 0: return e0; case 1: return car(xs); default: return std::accumulate(std::ranges::begin(cdr(xs) | as_proper_list), std::ranges::end(xs | as_proper_list), car(xs), gcd); } } },
      { "lcm",                           [](let const& xs) { switch (length(xs | as_proper_list)) { case 0: return e1; case 1: return car(xs); default: return std::accumulate(std::ranges::begin(cdr(xs) | as_proper_list), std::ranges::end(xs | as_proper_list), car(xs), lcm); } } },
      ...
      { "log",                           [](let const& xs) { switch (length(xs | as_proper_list)) { case 1: return log(car(xs)); case 2: return log(car(xs)) / log(cadr(xs)); default: throw error(make<string>("procedure log takes one or two arguments, but got"), xs); } } },
      ...
      { "atan",                          [](let const& xs) { switch (length(xs | as_proper_list)) { case 1: return atan(car(xs)); case 2: return atan2(car(xs), cadr(xs)); default: throw error(make<string>("procedure atan takes one or two arguments, but got"), xs); } } },
      ...
      { "bitwise-and",                   [](let const& xs) { return std::accumulate(std::ranges::begin(xs | as_proper_list), std::ranges::end(xs | as_proper_list), make<small_integer>(-1), bitwise_and); } },
      { "bitwise-ior",                   [](let const& xs) { return std::accumulate(std::ranges::begin(xs | as_proper_list), std::ranges::end(xs | as_proper_list), make<small_integer>(0), bitwise_ior); } },
      { "bitwise-xor",                   [](let const& xs) { return std::accumulate(std::ranges::begin(xs | as_proper_list), std::ranges::end(xs | as_proper_list), make<small_integer>(0), bitwise_xor); } },
      ...
      { "number->string",                [](let const& xs) { switch (length(xs | as_proper_list)) { case 1: return number_to_string(car(xs), 10); case 2: return number_to_string(car(xs), exact_integer_cast<std::size_t>(cadr(xs))); default: throw error(make<string>("procedure number->string takes one or two arugments, but got"), xs); } } },
      { "string->number",                [](let const& xs) { switch (length(xs | as_proper_list)) { case 1: return make_number(car(xs).as<string>().utf8(), 10); case 2: return make_number(car(xs).as<string>().utf8(), exact_integer_cast<std::size_t>(cadr(xs))); default: throw error(make<string>("procedure string->number takes one or two arugments, but got"), xs); } } },
      ...
      { "make-string",                   [](let const& xs) { switch (length(xs | as_proper_list)) { case 1: return make<string>(exact_integer_cast<std::size_t>(car(xs)), character()); case 2: return make<string>(exact_integer_cast<std::size_t>(car(xs)), cadr(xs).as<character>()); default: throw error(make<string>("procedure make-string takes one or two arugments, but got"), xs); } } },
      ...
      { "make-vector",                   [](let const& xs) { switch (length(xs | as_proper_list)) { case 1: return make<vector>(exact_integer_cast<std::size_t>(car(xs)), unspecified); case 2: return make<vector>(exact_integer_cast<std::size_t>(car(xs)), cadr(xs)); default: throw error(make<string>("procedure make-vector takes one or two arugments, but got"), xs); } } },
      ...
      { #TAG "vector-copy",              [](let const& xs) { auto copy = [&](std::size_t begin, std::size_t end) { assert(begin <= end); return make<TAG##vector>(car(xs).as<TAG##vector>().values[std::slice(begin, end - begin, 1)]); }; switch (length(xs | as_proper_list)) { case 1: return copy(0, car(xs).as<TAG##vector>().values.size()); case 2: return copy(exact_integer_cast<std::size_t>(cadr(xs)), car(xs).as<TAG##vector>().values.size()); case 3: return copy(exact_integer_cast<std::size_t>(cadr(xs)), exact_integer_cast<std::size_t>(caddr(xs))); default: throw error(make<string>("procedure " #TAG "vector-copy takes one to three arguments, but got"), xs); } } }, \
      { #TAG "vector-copy!",             [](let const& xs) { auto copy = [&](std::size_t at, std::size_t begin, std::size_t end) { assert(begin <= end); auto i = std::slice(at, end - begin, 1); auto j = std::slice(begin, end - begin, 1); car(xs).as_mutable<TAG##vector>().values[i] = caddr(xs).as<TAG##vector>().values[j]; }; switch (length(xs | as_proper_list)) { case 3: copy(exact_integer_cast<std::size_t>(cadr(xs)), 0, caddr(xs).as<TAG##vector>().values.size()); return unspecified; case 4: copy(exact_integer_cast<std::size_t>(cadr(xs)), exact_integer_cast<std::size_t>(cadddr(xs)), caddr(xs).as<TAG##vector>().values.size()); return unspecified; case 5: copy(exact_integer_cast<std::size_t>(cadr(xs)), exact_integer_cast<std::size_t>(cadddr(xs)), exact_integer_cast<std::size_t>(caddddr(xs))); return unspecified; default: throw error(make<string>("procedure " #TAG "vector-copy! takes three to five arguments, but got"), xs); } } }, \
