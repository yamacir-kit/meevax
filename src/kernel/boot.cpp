...
namespace meevax::inline kernel
{
  auto resolve(char const* name) -> void *
  {
    ...
    auto static const registry = std::unordered_map<std::string, procedure::signature>
    {
      ...
      { "gcd",                           [](let const& xs) { switch (length(xs)) { case 0: return e0; case 1: return car(xs); default: return std::accumulate(std::ranges::begin(cdr(xs) | as_proper_list), std::ranges::end(xs | as_proper_list), car(xs), gcd); } } },
      { "lcm",                           [](let const& xs) { switch (length(xs)) { case 0: return e1; case 1: return car(xs); default: return std::accumulate(std::ranges::begin(cdr(xs) | as_proper_list), std::ranges::end(xs | as_proper_list), car(xs), lcm); } } },
