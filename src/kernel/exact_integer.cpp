...
inline namespace kernel
{
  ...
  {
    auto deallocate = [](pointer_to<char> data)
    {
      using gmp_free_function = void (*)(pointer_to<void>, std::size_t);
      gmp_free_function current_free_function;
      mp_get_memory_functions(nullptr, nullptr, &current_free_function);
      std::invoke(current_free_function, static_cast<void_pointer>(data), std::strlen(data) + 1);
    };
