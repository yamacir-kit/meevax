...
int main(const int argc, char const* const* const argv) try
{
  ...
  {
    root.write_to(root.standard_debug_port(), meevax::header(__func__), "load ", each, "\n");
  ...
  if (root.in_interactive_mode())
  {
    ...
    {
      root.write_to(root.standard_interaction_port(), prompt);

      let const expression = root.read();

      root.write_to(root.standard_interaction_port(), "\n");

      // TODO MOVE INTO reader
      root.write_to(root.standard_debug_port(), meevax::header(__func__), expression, "\n");

      let const result = root.evaluate(expression);

      root.write_to(root.standard_interaction_port(), result, "\n");
    }
