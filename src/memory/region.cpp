...
{
  region::region(void * const base, std::size_t const size)
    : base { base }
    , size { size }
  {}
  ...
  auto region::contains(void * const derived) const noexcept -> bool
  {
  ...
  auto region::reset(void * const x, deallocator<void>::signature const f) noexcept -> region *
  {
    if (not assigned())
    {
      derived = x;
