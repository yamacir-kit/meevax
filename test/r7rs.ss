...
;   (import (scheme base))
;   (begin
;     (define (make n m) ; Create an NxM grid.
...
(check (real? +inf.0) => #t)
(check (real? +nan.0) => #t)
; (check (rational? -inf.0) => #f) ; BUG
; (check (rational? 3.5) => #t) ; BUG
...
(check (floor-quotient  5  2) =>  2) (check (floor-remainder  5  2) =>  1)
(check (floor-quotient -5  2) => -3) (check (floor-remainder -5  2) =>  1)
(check (floor-quotient  5 -2) => -3) (check (floor-remainder  5 -2) => -1)
(check (floor-quotient -5 -2) =>  2) (check (floor-remainder -5 -2) => -1)

(check (truncate-quotient  5    2) =>  2)   (check (truncate-remainder  5    2) =>  1)
(check (truncate-quotient -5    2) => -2)   (check (truncate-remainder -5    2) => -1)
(check (truncate-quotient  5   -2) => -2)   (check (truncate-remainder  5   -2) =>  1)
(check (truncate-quotient -5   -2) =>  2)   (check (truncate-remainder -5   -2) => -1)
(check (truncate-quotient -5.0 -2) =>  2.0) (check (truncate-remainder -5.0 -2) => -1.0)
...
(check (eq? (call-with-current-continuation
              (lambda (k)
