...
; begin boolean? caaaar caaadr caaar caadar caaddr caadr caar cadaar cadadr
...
; display do dynamic-wind eof-object? eq? equal? eqv? eval even? exact->inexact
; exact? exp expt floor for-each force gcd if imag-part inexact->exact inexact?
; input-port? integer->char integer? interaction-environment lambda lcm length
; let let* let-syntax letrec letrec-syntax list list->string list->vector
; list-ref list-tail list? load log magnitude make-polar make-rectangular
; make-string make-vector map max member memq memv min modulo negative? newline
...
; procedure? quasiquote quote quotient rational? rationalize read read-char
; real-part real? remainder reverse round scheme-report-environment set!
...
; tan truncate values vector vector->list vector-fill!  vector-length vector-ref
; vector-set! vector? with-input-from-file with-output-to-file write write-char
; zero?
